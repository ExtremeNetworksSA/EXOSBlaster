...
def switch_ssh(ip,cmds, device, mp_queue):
    ...
    try:
        ...
        time.sleep(1)
        resp = chan.recv(9999)	
        chan.send('disable cli paging\n')
        time.sleep(1)
        resp = chan.recv(9999)
        lines = resp.splitlines()
        switchname = lines[-1][:-1].decode('ascii','ignore')
    ...
    for i in range(0, len(ips), sizeofbatch):
        ...
        for device in batch:
            ...
            try:
                ...
                socket.inet_aton(ip)
                p = multiprocessing.Process(target=switch_ssh,args=(ip,cmds, device,mp_queue))
                processes.append(p)
                p.start()
            except socket.error:
